...
def show():
    ...
    top_asc_code = None
    if not branchWise_nonzero.empty:
        try:
            top_asc_code = int(str(branchWise_nonzero.iloc[0]["ASC name,code"]).split(", ")[-1])
        except (ValueError, IndexError):
            pass

    _asc_options = df["ASC Code"].unique().tolist()
    default_idx = (
        _asc_options.index(top_asc_code)
        if top_asc_code is not None and top_asc_code in _asc_options
        else 0
    )

    selected_asc = st.selectbox("Enter ASC Code", options=_asc_options, index=default_idx)
