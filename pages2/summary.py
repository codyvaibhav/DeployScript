...
def show():
    st.set_page_config(page_title=f"{config.main} Summary", page_icon="📋", layout="wide")
    ...
    st.markdown(
        f"<h1 style='text-align: center;'>{config.main} Model Comparison : {imp_overall} from its Base Model</h1>",
    ...
    with l:
        ...
        st.write("")
        html_table = df2.to_html(escape=False, index=False, classes="summary-table table3rd-col-bold")
        st.markdown(html_table, unsafe_allow_html=True)
        st.write("")
        overall_table, remarks_overall = generate_symptom_table(df, sell_out_overall)
        html_table = overall_table.to_html(escape=False, index=False, classes="summary-table table3rd-col-bold")
        st.markdown(html_table, unsafe_allow_html=True)
        st.markdown(
    ...
    with m:
        ...
        st.write("")
        html_table = df3.to_html(escape=False, index=False, classes="summary-table table3rd-col-bold")
        st.markdown(html_table, unsafe_allow_html=True)
        st.write("")
        domestic_df = df[df['RHQ'] == 'S.W.Asia']
        domestic_table, remarks_domestic = generate_symptom_table(domestic_df, sell_out_domestic)
        html_table = domestic_table.to_html(escape=False, index=False, classes="summary-table table3rd-col-bold")
        st.markdown(html_table, unsafe_allow_html=True)
        st.markdown(
    ...
    with r:
        ...
        st.write("")
        html_table = df4.to_html(escape=False, index=False, classes="summary-table table3rd-col-bold")
        st.markdown(html_table, unsafe_allow_html=True)
        st.write("")
        export_df = df[df['RHQ'] != 'S.W.Asia']
        export_table, remarks_export = generate_symptom_table(export_df, sell_out_export)
        html_table = export_table.to_html(escape=False, index=False, classes="summary-table table3rd-col-bold")
        st.markdown(html_table, unsafe_allow_html=True)
        st.markdown(
    ...
    with l:
        if total_days < 45:
            ...
            fig.data[0].name = f"{config.main}"   # ← config.main (live value)
            fig.data[1].name = f"{config.base}"   # ← config.base (live value)
            ...
            for i in indices:
                row = df5.iloc[i]
                fig.add_annotation(x=row['Day'], y=row['main_ratio'],
                                   text=f"{row['main_ratio']:.2f}", showarrow=False,
                                   yshift=-10, font=dict(size=10, color='blue'))
                fig.add_annotation(x=row['Day'], y=row['base_ratio'],
                                   text=f"{row['base_ratio']:.2f}", showarrow=False,
                                   yshift=10,  font=dict(size=10, color='#FC580A'))
            ...
            fig.data[0].name = f"{config.main}"   # ← live value
            fig.data[1].name = f"{config.base}"   # ← live value

            for i, row in df_weekly.iterrows():
                fig.add_annotation(x=row['Week'], y=row['main_ratio'],
                                   text=f"{row['main_ratio']:.2f}", showarrow=False,
                                   yshift=-15, font=dict(size=10, color='blue'))
                fig.add_annotation(x=row['Week'], y=row['base_ratio'],
                                   text=f"{row['base_ratio']:.2f}", showarrow=False,
                                   yshift=15, font=dict(size=10, color='#FC580A'))

            fig.update_layout(
                hovermode='x unified',
                xaxis=dict(title=None, showgrid=False, showline=False,
                           tickmode='array', showspikes=True,
                           spikemode='across', spikecolor='gray', spikethickness=1),
                yaxis=dict(showgrid=False, title=None),
                legend_title_text="",
                plot_bgcolor="#E8EDF2",
                xaxis_tickfont_family="Arial", xaxis_tickfont_size=12,
    ...
    with r:
        if symptom == 'Top 5':
            ...
            custom_colors = ['#540d6e', '#36369C', '#ffd23f', '#3bceac', '#0ead69']
            fig = px.line(
                df_weekly, x='WK', y='Count', color='Symptom Group 1',
            ...
            fig.update_xaxes(categoryorder='array',
                             categoryarray=sorted(df_weekly['WK'].unique(),
            ...
            fig = px.line(
                df_weekly, x='WK', y='Count',
                title=f'{symptom} Trend',
                labels={'Count': '', 'WK': ''},
                text='Count',
                color_discrete_sequence=['#36369C']
            )
            fig.update_traces(
                mode='lines+markers+text',
                textposition='top center',
